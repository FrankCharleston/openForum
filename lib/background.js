...
function logToOptionsPage(message) {
  // Sends log messages to the options page if debug mode is enabled.
...
// Override console.log to capture logs in your options page
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  try {
    // Do not inject on restricted URLs
...
function updateIcon(autoDecryptEnabled) {
  ...
  chrome.action.setIcon({ path: iconPath }, () => {
    if (chrome.runtime.lastError) {
      console.error("Setting icon error:", chrome.runtime.lastError.message);
...
function injectAndExecuteScript(tabId, func, args) {
  ...
  .catch((error) => {
    // If the error is due to restricted URLs, simply warn and exit.
    if (error.message && (error.message.includes("chrome://") || error.message.includes("edge://"))) {
      console.warn("Script injection skipped for restricted URL");
      return;
    }
...
function encryptSelectedText(text) {
  ...
  // Retrieve defaultPassphrase
  chrome.storage.local.get(["defaultPassphrase"], (data) => {
    let passphrase = data.defaultPassphrase;
    if (!passphrase) {
...
function decryptSelectedText(text) {
  ...
  chrome.storage.local.get(["defaultPassphrase"], (data) => {
    ...
    try {
      // Strip ENC[...] wrapper from the text
...
// ====== 8) Decrypt Text on Page Load ======
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete') {
    // Skip injection on restricted URLs
    if (!tab.url || tab.url.startsWith("chrome://") || tab.url.startsWith("edge://")) {
      console.warn("Auto decryption skipped for restricted URL:", tab.url);
      return;
    }
    chrome.storage.local.get(['autoDecrypt', 'defaultPassphrase'], (data) => {
      if (data.autoDecrypt && data.defaultPassphrase) {
        ...
        .catch((error) => {
          // Suppress errors if due to restricted URLs
          if (error.message && (error.message.includes("chrome://") || error.message.includes("edge://"))) {
            console.warn("Script injection skipped for restricted URL:", tab.url);
            return;
          }
          console.error("Error injecting script:", error);
          logToOptionsPage("Error injecting script: " + error.message);
