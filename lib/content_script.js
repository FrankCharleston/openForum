...
function decryptText(encryptedText, passphrase) {
  ...
  try {
    ...
    if (DEBUG_MODE) {
      logDebug("Decryption failed: Malformed UTF-8 data");
    }
...
async function scanAndDecryptPage() {
  if (DEBUG_MODE) logDebug("Starting scanAndDecryptPage...");
  await loadCryptoJS();
  const data = await getStorage(["defaultPassphrase"]);
  const storedPassphrase = data.defaultPassphrase || "";
  const passphrase = storedPassphrase || prompt("Enter passphrase to decrypt:");
  if (!passphrase) {
    if (DEBUG_MODE) logDebug("No passphrase provided. Aborting decryption.");
    return;
  }

  // Use a TreeWalker to scan all text nodes in <body> containing ENC[...]
  ...
  while ((node = walker.nextNode())) {
    ...
    while ((match = encRegex.exec(textContent)) !== null) {
      ...
      if (decrypted) {
        ...
        logDebug("Decryption failed for chunk: " + encryptedChunk);
      }
    }
    if (foundEncrypted) {
  ...
  if (DEBUG_MODE) logDebug("Finished scanning/decrypting.");
}

// =============== DYNAMICALLY LOAD CRYPTO-JS ===============
async function loadCryptoJS() {
  if (typeof CryptoJS === "undefined") {
    if (DEBUG_MODE) logDebug("Injecting crypto-js.min.js...");
    const script = document.createElement("script");
    script.src = chrome.runtime.getURL("lib/crypto-js.min.js");
    document.head.appendChild(script);
    await new Promise((resolve) => { script.onload = resolve; });
    if (DEBUG_MODE) logDebug("crypto-js.min.js loaded.");
  }
  // Ensure global encryption/decryption functions are set
