...
chrome.storage.local.get(["autoDecrypt", "debugMode", "redditUsername"], (data) => {
  ...
  if (AUTO_DECRYPT) {
    ...
    scanAndDecryptPage(data.redditUsername);
  } else {
  ...
  if (message.action === "toggleDecryption") {
    ...
    loadCryptoJS().then(() => {
      chrome.storage.local.get(["defaultPassphrase", "redditUsername"], (data) => {
        ...
        const userPassphrase = data.redditUsername || storedPassphrase || prompt("Enter decryption passphrase:");
...
async function scanAndDecryptPage(redditUsername) {
  ...
  chrome.storage.local.get("defaultPassphrase", (data) => {
    ...
    const passphrase = redditUsername || storedPassphrase || prompt("Enter passphrase to decrypt:");
    if (!passphrase) {
  ...
  if (typeof window.decryptText === "undefined") {
    window.decryptText = function (encryptedText, passphrase) {
      ...
      try {
        ...
        if (!decrypted) {
          console.warn("Decryption resulted in empty string. Likely incorrect passphrase.");
          return null;
        }
        return decrypted;
      } catch (error) {
...
function getUserIdFromComment(text) {
  // Implement logic to extract user ID from the comment text
  // This is a placeholder function and should be replaced with actual logic
  const userIdMatch = text.match(/user_id:\s*(\w+)/);
...
//  crediting @bobsteger for identifying the issue https://github.com/FrankCharleston/openForum/issues/14